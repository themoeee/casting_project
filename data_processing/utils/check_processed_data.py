...
try:
    ...
    sys.path.append(str(Path(__file__).resolve().parent.parent / "processing"))
...
FINAL_INPUT_DATA_ROOT = PROCESSED_ROOT / "final_input_data"
LONG_FORM_ROOT = PROCESSED_ROOT / "long_form"
PROCESSING_INFO_ROOT = PROCESSED_ROOT / "processing_info"
RAW_UT_ROOT = RAW_ROOT / "ut_tests"

DEFAULT_MASTER_PARQUET = FINAL_INPUT_DATA_ROOT / "master_sample_table.parquet"
DEFAULT_MASTER_CSV = FINAL_INPUT_DATA_ROOT / "master_sample_table.csv"
DEFAULT_CAVITY_PARQUET = LONG_FORM_ROOT / "cavity_sensor_data.parquet"
DEFAULT_CAVITY_MANIFEST = PROCESSING_INFO_ROOT / "cavity_sensors_manifest.parquet"
DEFAULT_REMOVED_CAVITY_CSV = PROCESSING_INFO_ROOT / "removed_cavity_sensor_cycles.csv"
DEFAULT_DDM_PARQUET = LONG_FORM_ROOT / "ddm_machine_data.parquet"
DEFAULT_DDM_MANIFEST = PROCESSING_INFO_ROOT / "ddm_manifest.parquet"
DEFAULT_REMOVED_DDM_CSV = PROCESSING_INFO_ROOT / "removed_ddm_cycles.csv"
DEFAULT_UT_PARQUET = FINAL_INPUT_DATA_ROOT / "fake_ut_test_results.parquet"
DEFAULT_UT_CSV = FINAL_INPUT_DATA_ROOT / "fake_ut_test_results.csv"
DEFAULT_RAW_UT_PARQUET = RAW_UT_ROOT / "fake_ut_test_results.parquet"
DEFAULT_RAW_UT_CSV = RAW_UT_ROOT / "fake_ut_test_results.csv"
...
def check_ml_input_readiness() -> dict[str, Any]:
    ...
    message = (
        "No error found in data preprocessing."
        if ready
        else "Data is not complete. See issues and missing_by_label."
    )
